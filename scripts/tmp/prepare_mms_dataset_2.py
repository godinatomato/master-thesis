...
root_dir = Path.cwd()
output_dir = root_dir / "assets/Sentiment_mms"
output_dir.mkdir(parents=True, exist_ok=True)

dataset = load_dataset("Brand24/mms")
subset = dataset.filter(lambda row: row["language"] in LANGS)

raw_data = {}
for label_name, label in LABELS.items():
    raw_data[label_name] = subset.filter(lambda row: row["label"] == label)["train"][
        "text"
    ]
