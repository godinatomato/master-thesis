...
def argument_parser(prev_args: t.Optional[str] = None):
    ...
    parser.add_argument(
        "--expertise", type=pathlib.Path, help="Expertise results as CSV file."
    )
    parser.add_argument(
        "--metric",
        type=str,
    ...
    parser.add_argument(
        "--forcing", type=str, nargs="*", default=["on_p50"], help="Forcing value."
    )
    parser.add_argument(
        "--num-units",
        type=int,
    ...
    parser.add_argument(
        "--no-save", action="store_true", help="If set, nothing is saved."
    )
    parser.add_argument(
        "--only-last-token",
        action="store_true",
...
def generate(args):
    ...
    if args.results_file is None:
        results_file: pathlib.Path = (
            args.expertise.parent
            / f'forced_sentences_{concept}_{args.prompt.replace("_", "")}.csv'
        )
    else:
    ...
    if not args.no_save:
        ...
        for units, units_df in generated_df.groupby(by="num_units", sort=False):
            for i, sentence in zip(
                range(len(generated_df)), units_df["sentence"].values
            ):
